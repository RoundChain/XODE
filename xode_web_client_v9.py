    ...
    def _handle_new_block(self, msg):
        ...
        if supply:
            ...
            if "burned_total" in supply and supply["burned_total"] is not None:
                try:
                    self.burned_total = float(supply["burned_total"])
                except (ValueError, TypeError):
                    pass
...
rewardHtml+='<div style="font-size:11px;color:#00d4ff;font-weight:600;margin-bottom:6px">🎁 Block Reward: '+perUser.toFixed(2)+' XODE x '+recipients.length+' users</div>';
...
const amt=typeof r==='object'?(r.amount||perUser).toFixed(2):perUser.toFixed(2);
...
html+='<span>⏰ '+date+'</span>';
if(supply.issued){html+='<span>📊 '+supply.issued.toLocaleString()+' / '+(supply.total?supply.total.toLocaleString():'?')+'</span>';}
if(supply.burned_total||supply.burned_total===0){html+='<span>🔥 Burned: '+(supply.burned_total||0).toLocaleString()+'</span>';}
