    ...
    def request_sync(self):
        ...
        if not has_genesis:
            ...
            while start <= target_height and self.running:
                ...
                self.add_log(f"Requesting blocks #{start} to #{end-1}...")
                start = end
                time.sleep(0.5)
            wait_count = 0
            while self.chain_store.get_local_height() < target_height and wait_count < 50:
                time.sleep(0.3)
                wait_count += 1
            self.syncing = False
            ...
            try:
                ...
                while buffer:
                    try:
                        ...
                        # 无法恢复，清空 buffer
                        if len(buffer) > 65536:
                            self.add_log(f"Buffer overflow, clearing {len(buffer)} bytes", "warning")
                            buffer = b""
                        break
                    except Exception as e:
    ...
    def handle_message(self, msg):
        ...
        if msg_type == "pong":
            ...
            if blocks:
                ...
                self.add_log(f"Added {added} blocks, local height now: #{self.chain_store.get_local_height()}")
                # 更新最新区块的分奖地址
            ...
            if local_height >= 0 and server_height > local_height and not self.syncing:
                ...
                self.add_log(f"Auto sync: behind by {missing} blocks, requesting from server...", "warning")
                threading.Thread(target=self.request_sync, daemon=True).start()
